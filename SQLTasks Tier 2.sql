...
FROM Facilities
WHERE membercost > 0
AND (membercost * 5) < monthlymaintenance;
...
SELECT DISTINCT CONCAT_WS(' ',firstname, surname) AS membername, Facilities.name AS facility
FROM Members
JOIN Bookings
...
SELECT f.name,
CONCAT_WS(' ', firstname, surname) AS membername,
...
SELECT x.facility_name, x.membername, x.cost
FROM (
...
SELECT result.Facility_name, result.revenue
FROM (
...
ORDER BY revenue DESC) as result
WHERE revenue < 1000

/* Q11: Produce a report of members and who recommended them in alphabetic surname,firstname order */

SELECT CONCAT_WS( ' ', m.firstname, m.surname ) AS membername,
CONCAT_WS( ' ', spon.firstname, spon.surname ) AS sponsor
FROM Members AS m
INNER JOIN Members AS spon
ON m.recommendedby = spon.memid
WHERE CONCAT_WS( ' ', spon.firstname, spon.surname ) NOT LIKE '%GUEST%'
ORDER BY sponsor
